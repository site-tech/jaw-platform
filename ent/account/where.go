...
// HasTennants applies the HasEdge predicate on the "tennants" edge.
func HasTennants() predicate.Account {
	return predicate.Account(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(TennantsTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, TennantsTable, TennantsColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasTennantsWith applies the HasEdge predicate on the "tennants" edge with a given conditions (other predicates).
func HasTennantsWith(preds ...predicate.Tennant) predicate.Account {
	return predicate.Account(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(TennantsInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, TennantsTable, TennantsColumn),
		)
