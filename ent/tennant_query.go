...
type TennantQuery struct {
	...
	loadTotal   []func(context.Context, []*Tennant) error
...
func (tq *TennantQuery) QueryAccount() *AccountQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(tennant.Table, tennant.FieldID, selector),
...
// First returns the first Tennant entity from the query.
// Returns a *NotFoundError when no Tennant was found.
func (tq *TennantQuery) First(ctx context.Context) (*Tennant, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{tennant.Label}
	}
	return nodes[0], nil
}
...
// FirstID returns the first Tennant ID from the query.
// Returns a *NotFoundError when no Tennant ID was found.
...
// Only returns a single Tennant entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one Tennant entity is found.
// Returns a *NotFoundError when no Tennant entities are found.
func (tq *TennantQuery) Only(ctx context.Context) (*Tennant, error) {
	...
	case 0:
		return nil, &NotFoundError{tennant.Label}
	default:
		return nil, &NotSingularError{tennant.Label}
	}
}
...
// OnlyID is like Only, but returns the only Tennant ID in the query.
// Returns a *NotSingularError when more than one Tennant ID is found.
...
func (tq *TennantQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	...
	case 0:
		err = &NotFoundError{tennant.Label}
	default:
		err = &NotSingularError{tennant.Label}
	}
	return
}
...
func (tq *TennantQuery) IDs(ctx context.Context) ([]uuid.UUID, error) {
	var ids []uuid.UUID
	if err := tq.Select(tennant.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}
...
// Clone returns a duplicate of the TennantQuery builder, including all associated steps. It can be
...
func (tq *TennantQuery) Clone() *TennantQuery {
	...
	return &TennantQuery{
		...
		predicates:  append([]predicate.Tennant{}, tq.predicates...),
...
func (tq *TennantQuery) GroupBy(field string, fields ...string) *TennantGroupBy {
	grbuild := &TennantGroupBy{config: tq.config}
...
func (tq *TennantQuery) Select(fields ...string) *TennantSelect {
	...
	selbuild := &TennantSelect{TennantQuery: tq}
	selbuild.label = tennant.Label
	selbuild.flds, selbuild.scan = &tq.fields, selbuild.Scan
	return selbuild
}

// Aggregate returns a TennantSelect configured with the given aggregations.
func (tq *TennantQuery) Aggregate(fns ...AggregateFunc) *TennantSelect {
	return tq.Select().Aggregate(fns...)
}

func (tq *TennantQuery) prepareQuery(ctx context.Context) error {
...
func (tq *TennantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tennant, error) {
	var (
		nodes       = []*Tennant{}
		_spec       = tq.querySpec()
		loadedTypes = [1]bool{
	...
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*Tennant).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &Tennant{config: tq.config}
	...
	if query := tq.withAccount; query != nil {
		if err := tq.loadAccount(ctx, query, nodes, nil,
			func(n *Tennant, e *Account) { n.Edges.Account = e }); err != nil {
			return nil, err
		}
	}
...
func (tq *TennantQuery) loadAccount(ctx context.Context, query *AccountQuery, nodes []*Tennant, init func(*Tennant), assign func(*Tennant, *Account)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Tennant)
	for i := range nodes {
...
func (tq *TennantQuery) sqlCount(ctx context.Context) (int, error) {
	_spec := tq.querySpec()
	if len(tq.modifiers) > 0 {
		_spec.Modifiers = tq.modifiers
	}
...
func (tq *TennantQuery) sqlExist(ctx context.Context) (bool, error) {
...
func (tq *TennantQuery) querySpec() *sqlgraph.QuerySpec {
...
func (tq *TennantQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(tq.driver.Dialect())
	t1 := builder.Table(tennant.Table)
	columns := tq.fields
	if len(columns) == 0 {
		columns = tennant.Columns
	}
...
func (tgb *TennantGroupBy) Scan(ctx context.Context, v any) error {
...
func (tgb *TennantGroupBy) sqlScan(ctx context.Context, v any) error {
...
func (tgb *TennantGroupBy) sqlQuery() *sql.Selector {
...
func (ts *TennantSelect) Aggregate(fns ...AggregateFunc) *TennantSelect {
	ts.fns = append(ts.fns, fns...)
	return ts
}

// Scan applies the selector query and scans the result into the given value.
func (ts *TennantSelect) Scan(ctx context.Context, v any) error {
	if err := ts.prepareQuery(ctx); err != nil {
		return err
	}
	ts.sql = ts.TennantQuery.sqlQuery(ctx)
	return ts.sqlScan(ctx, v)
}

func (ts *TennantSelect) sqlScan(ctx context.Context, v any) error {
