...
type AccountQuery struct {
	...
	withTennants      *TennantQuery
	modifiers         []func(*sql.Selector)
	loadTotal         []func(context.Context, []*Account) error
	withNamedTennants map[string]*TennantQuery
...
func (aq *AccountQuery) QueryTennants() *TennantQuery {
	query := &TennantQuery{config: aq.config}
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withTennants: aq.withTennants.Clone(),
...
// WithTennants tells the query-builder to eager-load the nodes that are connected to
// the "tennants" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithTennants(opts ...func(*TennantQuery)) *AccountQuery {
	query := &TennantQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	aq.withTennants = query
	return aq
}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withTennants; query != nil {
		if err := aq.loadTennants(ctx, query, nodes,
			func(n *Account) { n.Edges.Tennants = []*Tennant{} },
			func(n *Account, e *Tennant) { n.Edges.Tennants = append(n.Edges.Tennants, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range aq.withNamedTennants {
		if err := aq.loadTennants(ctx, query, nodes,
			func(n *Account) { n.appendNamedTennants(name) },
			func(n *Account, e *Tennant) { n.appendNamedTennants(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (aq *AccountQuery) loadTennants(ctx context.Context, query *TennantQuery, nodes []*Account, init func(*Account), assign func(*Account, *Tennant)) error {
	...
	query.Where(predicate.Tennant(func(s *sql.Selector) {
		s.Where(sql.InValues(account.TennantsColumn, fks...))
	}))
...
// WithNamedTennants tells the query-builder to eager-load the nodes that are connected to the "tennants"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithNamedTennants(name string, opts ...func(*TennantQuery)) *AccountQuery {
	query := &TennantQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
	if aq.withNamedTennants == nil {
		aq.withNamedTennants = make(map[string]*TennantQuery)
	}
	aq.withNamedTennants[name] = query
	return aq
}
